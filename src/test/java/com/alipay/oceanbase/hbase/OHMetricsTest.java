...
public class OHMetricsTest {
    ...
    * */
    ...
    public void testGetListOperationsMetrics() throws Exception {
        ...
        hTable.put(put2);
        hTable.put(put3);
        List<Get> gets = Arrays.asList(new Get(Bytes.toBytes(row1)), new Get(Bytes.toBytes(row2)),
    ...
    public void testCheckAndMutateOperationsMetrics() throws Exception {
        ...
        boolean result = hTable
            .checkAndMutate(Bytes.toBytes(row1), Bytes.toBytes(family1), Bytes.toBytes("q1"),
                CompareFilter.CompareOp.EQUAL, Bytes.toBytes("val1"), rowMutations);
        Assert.assertTrue(result);
        ...
        try {
            ...
            notExistTable.checkAndMutate(Bytes.toBytes(row1), Bytes.toBytes(family1),
                Bytes.toBytes("q1"), CompareFilter.CompareOp.EQUAL, Bytes.toBytes("val1"),
                rowMutations);
            fail();
        } catch (Exception e) {
    ...
    public void testIncrementColumnValueOperationsMetrics() throws Exception {
        ...
        MetricsExporter oldExporter = metrics
            .acquireMetrics(OHOperationType.INCREMENT_COLUMN_VALUE);

        long res = hTable.incrementColumnValue(Bytes.toBytes(row1), Bytes.toBytes(family1),
            Bytes.toBytes("q1"), 10L);
        Assert.assertEquals(10L, res);

        Thread.sleep(11000); // sleep over 10 second
        MetricsExporter newExporter = metrics
            .acquireMetrics(OHOperationType.INCREMENT_COLUMN_VALUE);
        ...
        try {
            Table notExistTable = connection.getTable(TableName.valueOf("not_exist_table"));
            notExistTable.incrementColumnValue(Bytes.toBytes(row1), Bytes.toBytes(family1),
                Bytes.toBytes("q1"), 10L);
            fail();
        } catch (Exception e) {
        ...
        Thread.sleep(11000); // sleep over 10 second
        MetricsExporter failedExporter = metrics
            .acquireMetrics(OHOperationType.INCREMENT_COLUMN_VALUE);
    ...
    private void metricsChecker(OHOperationType type, MetricsExporter oldExporter,
        ...
        System.out.println(String.format("%s - current AverageOps: " + newExporter.getAverageOps()
                                         + ", before AverageOps: " + oldExporter.getAverageOps(),
            type.name()));
        ...
        System.out.println(String.format(
            "%s - current AverageSingleOpCount: " + newExporter.getAverageSingleOpCount()
                    + ", before AverageSingleOpCount: " + oldExporter.getAverageSingleOpCount(),
            type.name()));
        Assert.assertTrue(String.format("%s average single op count should > 0", type.name()),
            newExporter.getAverageSingleOpCount() > 0);

        System.out.println(String.format(
            "%s - current AverageLatency: " + newExporter.getAverageLatency()
        ...
        System.out.println(String.format("%s - current MaxLatency: " + newExporter.getMaxLatency()
                                         + ", before MaxLatency: " + oldExporter.getMaxLatency(),
            type.name()));
        Assert.assertTrue(String.format("%s max latency should > 0", type.name()),
            newExporter.getMaxLatency() > 0);

        System.out.println(String.format(
            "%s - current 99thPercentile: " + newExporter.get99thPercentile()
        ...
        System.out.println(String.format(
            "%s - current TotalRuntime: " + newExporter.getTotalRuntime()
    ...
    private void failureMetricsChecker(OHOperationType type, MetricsExporter oldExporter,
                                       MetricsExporter newExporter) throws Exception {
        Assert.assertEquals(String.format("%s initial failed op count should be 0", type.name()),
            0L, oldExporter.getFailCount());
        Assert.assertEquals(String.format("%s failed op count should be 1", type.name()), 1,
            newExporter.getFailCount());
    }
}