...
public class OHTableGetOptimizeTest {
    ...
    public void testGetOptimizeWithoutQualifierAndExplicitMaxVersion1() throws Exception {
        ...
        for (int i = 1; i <= 5; i++) {
            ...
            put.add(family.getBytes(), col2.getBytes(), ts, toBytes("q2_v" + i));
            put.add(family.getBytes(), col3.getBytes(), ts, toBytes("q3_v" + i));
            put.add(family.getBytes(), col4.getBytes(), ts, toBytes("q4_v" + i));
            hTable.put(put);
            Thread.sleep(5);
        }
    ...
    public void testGetOptimizeWithoutQualifierTimestampAndMaxVersion1() throws Exception {
        ...
        for (int i = 0; i < 5; i++) {
            timestamps[i] = baseTime + (i * 100);
            Put put = new Put(toBytes(key));
            put.add(family.getBytes(), col1.getBytes(), timestamps[i], toBytes("a_v" + i));
            put.add(family.getBytes(), col2.getBytes(), timestamps[i], toBytes("b_v" + i));
            put.add(family.getBytes(), col3.getBytes(), timestamps[i], toBytes("c_v" + i));
            put.add(family.getBytes(), col4.getBytes(), timestamps[i], toBytes("d_v" + i));
            hTable.put(put);
            Thread.sleep(5);
        }
        ...
        for (int i = 0; i < r2.raw().length; i++) {
            Assert.assertEquals(timestamps[4], r2.raw()[i].getTimestamp());
        }
    }
}
